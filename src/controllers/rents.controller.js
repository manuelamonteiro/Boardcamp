...
export async function postRent(req, res) {
    ...
    try {
        ...
        await connectionDB.query(`UPDATE games SET "stockTotal"=$1 WHERE id=$2;`, [newStock, gameId]);
...
export async function endRent(req, res) {
    ...
    try {
        ...
        const newStock = Number(game.rows[0].stockTotal) + 1;
        await connectionDB.query(`UPDATE games SET "stockTotal"=$1 WHERE id=$2;`, [newStock, game.rows[0].id]);
